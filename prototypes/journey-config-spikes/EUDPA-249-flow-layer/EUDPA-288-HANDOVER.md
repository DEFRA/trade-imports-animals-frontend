...
- **[DONE 2026-07-20] Accompanying documents reshaped from WS2 all-or-nothing block into a 0..10 user-driven indexed group.** Confluence page 6497338582 is stale on this — the team confirmed 0..10 documents per notification (PO to update page later). Reversed the WS2 "scalar-invariant landing" locked decision; the deferred records-shape upgrade flagged in `PLAN-accompanying-doc-block.md` arrived as WS4. `accompanyingDocument` is now a records-shape group (same category as `commodityLine`) with four member fields `within: accompanyingDocument`, `status: 'mandatory'`. New `requires.maxEntries: 10` invariant kind (fifth on `groupInvariantErrors`) — cap enforced at UI (Add button greyed at 10) and at rollup (redeploy-safety net). New `features/accompanying-documents/` feature dir mirroring `features/commodity-lines/`; `lib/accompanying-doc-page-controller.js` sibling to `line-page-controller.js`; `lib/state.js` gained `addAccompanyingDocument` / `deleteAccompanyingDocument`. CYA prompt for `MAX_ENTRIES` replaces the retired allOrNothing prompt. Plan preserved in `PLAN-accompanying-doc-indexed.md`. Confluence page divergence flagged in `obligations.js` header comment so a future spec-vs-model comparison doesn't re-derive the wrong conclusion.
