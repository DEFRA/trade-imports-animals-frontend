...
| `consignment`          | import reason, import purpose, destination country, port of exit, exit date, additional details |
| `documents`            | documents                                                                                       |
...
- `sectionObligationIds(section)` / `sectionStatus(section, answers, inScope, evaluation)` — the section-shaped equivalents, consumed by the review row and `analysis/simulate.js`.
- `readyForCheckYourAnswers(answers, inScope, evaluation)` (`flow/section-status.js`) — the submit-readiness gate: true once every task row is Fulfilled, Not applicable or Optional. Consulted both by the review section's authored gate and by `submitJourney` in `engine/write/index.js`.
