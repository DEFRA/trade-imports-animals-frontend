...
Precedence extends the established chain by one link: **hub exit > change context > run sequence > `nextInSection`**. `kit.nextTarget` consults `kit.runTarget` (null unless the run is active for the current journey) before `nextInSection`; the identifier-list controller consults it before its own loop fallback. With no session record every redirect is byte-for-byte the pre-run behaviour.
