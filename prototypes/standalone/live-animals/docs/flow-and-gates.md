...
- **`flow/run-state.js` — session-side presentation state.** Run position is stateless (derived from the page being posted); completion is a session record `{ journeyId, phase: active|complete }` behind the SESSION port (`beginOpeningRun`/`completeOpeningRun`/`inOpeningRun`; cookie in stub mode, yar in real mode). It cannot be derived from answers: a zero-record identification pass leaves no footprint, and `importType` never survives a real-mode round-trip. The completed record persists as the "entered through the filter" memory the entry guard needs in real mode (`hasEnteredThroughFilter`).
- **`flow/entry-guard.js` — the deep-link guard.** A plugin-level `onPreHandler` sends a fresh journey — no committed notification answers and never through the filter — from any post-filter journey page to the filter. Both the guard and the genuine-start check use `hasCommittedNotificationAnswers`, which counts only answers the USER entered: `userEntered(key)` requires the key to be a page-collected obligation (`obligationByName`, not in `SYSTEM_POPULATED`) and not `importType`. Two kinds of key are therefore ignored. `importType` is the filter's own service-routing pick, not a start, and it never survives a real-mode round-trip. Keys that are not obligations belong to the BACKEND: real mode rebuilds `answers` from the stored notification on every load, so a freshly-created DRAFT arrives already carrying its server-minted `referenceNumber`. Counting either would make stub and real mode diverge — real mode would treat every new journey as already started, never open the run and never guard a deep link. Exempt: the dashboard, the filter and `start` (`EXEMPT_PREFIXES`). After the first notification commit (or with a recorded filter pass) deep links behave normally. This guard is also why `importType` carries no obligation mandate: the field is enforced by entry routing plus the controller's `requiredOneOf`, never by the obligation model.
...
- `rowStatus(row, answers, inScope)` — the engine's pure `statusOfFromB` applied to the row's parts.
