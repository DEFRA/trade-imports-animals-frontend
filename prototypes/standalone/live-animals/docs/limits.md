...
- **Required-sibling item-conditionality feeding completeness.** `windscreenProvider` was REQUIRED, so its gate fed `entryComplete` and could hold a section In progress / lock `readyForQuote` (submit readiness). `numberOfPackages` is OPTIONAL, so no live item-conditional field gates completeness.
