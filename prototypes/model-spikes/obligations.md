...
| **Type**                                                             | A single data type (the type of what gets produced). Open type space — `date`, `string`, `email`, `file`, `boolean`, `address`, `monetaryAmount` (stored as text to preserve pounds-and-pence literally), `payment-receipt`, `sub-journey-receipt`, `lookup-result`, etc.                                         |
