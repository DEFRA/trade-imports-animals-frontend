...
const handleRemove = async (request, h, uploadId, traceId) => {
...
const collectValidationErrors = (payload, fileData) => {
  ...
  if (!hasFile) {
    ...
    fileErrors.push({
      message: 'The selected file must be a PDF, DOC, DOCX, JPEG, PNG or XLS',
      path: ['file'],
      type: 'any.invalid',
      context: { label: 'file', key: 'file' }
    })
  ...
  return { schemaErrors, dateErrors, fileErrors }
}

const uploadFile = async (request, fileData, uploadDetails, traceId) => {
...
export const postHandler = async (request, h) => {
  ...
  if (_action?.startsWith('remove-')) {
    ...
    return handleRemove(request, h, uploadId, traceId)
  }
  ...
  const { schemaErrors, dateErrors, fileErrors } = collectValidationErrors(
    request.payload,
    fileData
  )
  ...
  let uploadId
  try {
    uploadId = await uploadFile(
      request,
      fileData,
      { documentType, documentReference, dateOfIssue },
      traceId
    )
  } catch (err) {
